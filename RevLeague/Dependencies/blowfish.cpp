...
{
	return ((m_auiS[0][LOBYTE(ui >> 24)] + m_auiS[1][LOBYTE(ui >> 16)]) ^ m_auiS[2][LOBYTE(ui >> 8)]) + m_auiS[3][LOBYTE(ui)];
}
...
inline static void BytesToBlock(unsigned char const* p, SBlock& b)
{
	unsigned int y;
	b.m_uil = 0;
	y = *p++;
...
inline static void BlockToBytes(SBlock const& b, unsigned char* p)
{
	unsigned int y;
	y = b.m_uir;
	*--p = LOBYTE(y);
...
void CBlowFish::Encrypt(unsigned char* buf, size_t n)
{
	SBlock work;
	for (; n >= 8; n -= 8)
	{
...
void CBlowFish::Decrypt(unsigned char* buf, size_t n)
{
	SBlock work;
	for (; n >= 8; n -= 8)
	{
